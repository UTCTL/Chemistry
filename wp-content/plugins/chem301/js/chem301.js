    ...
    */
    ...
    if (typeof search_page !== "undefined") {
    	...
    	$(document).ready( function() {
            ...
            var toggle_buttons = function($box1, $box2) {
                $box1.animate({
                    opacity:'toggle'
                },300);
                $box2.animate({
            ...
            var video_open = function(self, oembed) {
                width = Math.min(660, oembed.width);
                ratio = width/oembed.width;
                height = oembed.height*ratio;
                
                $box = self.parent().parent();
                $li = $box.parent()
                $close = $li.find('.close_button');
                $li.animate({
                    'margin-left':0
                });
                $li.find('.thumb').animate({
                    width:width+'px',
                    height:height+'px'
                }, function(){
                    $(this).replaceWith(oembed.html);
                    toggle_buttons($box,$close);
                });
            };
            
            var video_close = function($self,oembed) {
                width = 200;
                ratio = width/oembed.thumbnail_width;
                height = oembed.thumbnail_height*ratio;
                
                $li = self.parent();
                $box = $li.find('.info_overlay');
                $close = $li.find('.close_button');
                $li.animate({
                    'margin-left':'10px'
                });
                $obj = $li.find('object');
                _width = $obj.width();
                _ratio2 = oembed.height/oembed.width;
                _height = _width*_ratio2;
                $obj.replaceWith('<img class="thumb" style="width:'+_width+'px;height:'+_height+'px;" src="'+oembed.thumbnail_url+'"></img>');
                toggle_buttons($box,$close);
                $li.find('.thumb').animate({
                                        width:width+'px',
                                        height:height+'px'
            ...
            $('.module .videos li .play a').bind('click',function(e){
                e.preventDefault();
                self = $(this);
                _id = self.parent().parent().parent().attr('id');
                video_open(self,videos[_id]);
            });
            $('.module .videos li .close_button').bind('click',function(e){
                e.preventDefault();
                self = $(this);
                _id = self.parent().attr('id');
                video_close(self,videos[_id]);
            });
            */
